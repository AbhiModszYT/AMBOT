...
from AMBOT.utils.plug import load_module, plug_channel
from AMBOT.utils.startup import (join_it, logger_check, start_msg,
                                        update_sudo)
from AMBOT.version import __hellver__

# Global Variables #
HELL_PIC = "https://telegra.ph/file/423a5a5a1f136a2258b76.mp4"
    ...
    for name in files:
        with open(name) as hell:
            ...
            if shortname.replace(".py", "") in Config.UNLOAD:
                os.remove(Path(f"AMBOT/plugins/{shortname}.py"))
            else:
...
async def start_hellbot():
    try:
        ...
        Hell.tgbot = HellBot
        LOGS.info("••• Starting AMBOT (TELETHON) •••")
        ...
        LOGS.info("••• AMBOT Startup Completed •••")
