...
@hell_cmd(pattern="kya$")
async def _(event):
    event = await eor(event, "kya bola isne ")
    deq = deque(list("🤔🧐🤔🧐🤔🧐"))
    for _ in range(48):
...
@hell_cmd(pattern="lol$")
async def _(event):
    event = await eor(event, "Ha Ha...")
    deq = deque(list("😂🤣😂🤣😂🤣"))
    for _ in range(48):
        await asyncio.sleep(0.1)
        await event.edit("".join(deq))
