...
from AMBOT.DB import pmpermit_sql as pm_sql
...
if str(Config.PM_PERMIT).lower() not in disabled_list:
    ...
    @bot.on(admin_cmd(pattern="(ok|a|approve|allow)$"))
    ...
    @bot.on(admin_cmd(pattern="(no|da|disapprove|disallow)$"))
    ...
    if H2:
        ...
        @H2.on(admin_cmd(pattern="(ok|a|approve|allow)$"))
        async def approve(event):
            if event.is_private:
                getuser = await event.client(
                    GetFullUserRequest(await event.get_input_chat())
        ...
        @H2.on(admin_cmd(pattern="(no|da|disapprove|disallow)$"))
        async def disapprove(event):
            if event.is_private:
                getuser = await event.client(
                    GetFullUserRequest(await event.get_input_chat())
    ...
    if H3:
        ...
        @H3.on(admin_cmd(pattern="(ok|a|approve|allow)$"))
        async def approve(event):
            if event.is_private:
                getuser = await event.client(
                    GetFullUserRequest(await event.get_input_chat())
        ...
        @H3.on(admin_cmd(pattern="(no|da|disapprove|disallow)$"))
        async def disapprove(event):
            if event.is_private:
                getuser = await event.client(
                    GetFullUserRequest(await event.get_input_chat())
    ...
    if H4:
        ...
        @H4.on(admin_cmd(pattern="(no|da|disapprove|disallow)$"))
        async def disapprove(event):
            if event.is_private:
                getuser = await event.client(
                    GetFullUserRequest(await event.get_input_chat())
    ...
    if H5:
        ...
        @H5.on(admin_cmd(pattern="(ok|a|approve|allow)$"))
        async def approve(event):
            if event.is_private:
                getuser = await event.client(
                    GetFullUserRequest(await event.get_input_chat())
        ...
        @H5.on(admin_cmd(pattern="(no|da|disapprove|disallow)$"))
        async def disapprove(event):
            if event.is_private:
                getuser = await event.client(
                    GetFullUserRequest(await event.get_input_chat())
...
@hell_cmd(pattern="alist|listapproved$")
...
CmdHelp("pm_permit").add_command(
    "a|ok|allow", "<in pm>", "Approves the user in which pm cmd is used."
).add_command(
    "no|da|disallow", "<in pm>", "Disapprove User to PM you."
).add_command(
    "block", "<in pm>/<reply>", "Blocks the user"
).add_command(
    "unblock", "<in pm>/<reply>", "Unblocks the mentioned user."
).add_command(
    "alist|listapproved", None, "Sends the list of all users approved by AMBOT"
).add_info(
    "PM SECURITY"
).add_warning(
    "✅ Harmless Module."
