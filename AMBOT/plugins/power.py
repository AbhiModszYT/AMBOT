...
import heroku3
from AMBOT.DB.gvar_sql import addgvar, delgvar, gvarstat
...
async def restart(event):
    if Config.HEROKU_APP_NAME and Config.HEROKU_API_KEY:
        ...
        await eor(event, f"✅ **Restarted AMBOT** \n**Type** `{hl}ping` **after 1 minute to check if I am working !**")
        await event.client.disconnect()


@hell_cmd(pattern="restart$")
async def re(hell):
    event = await eor(hell, "Restarting AMBOT ...")
    try:
        await restart(event)
...
async def rel(event):
    await eor(event, "**Reloaded AMBOT!** \n\n__This might take a minute.__")
    await reload_hellbot()


@hell_cmd(pattern="shutdown$")
async def down(event):
    await eor(event, "**[ ⚠️ ]** \n**AMBOT is now turned off. Manually turn it on to start again.**")
...
CmdHelp("power").add_command(
    ...
    "svar", "<variable name> <variable value>", "Sets the variable to SQL variables without restarting the bot.", "svar ALIVE_PIC https://telegra.ph/file/9470b838234d9535b0f82.jpg"
).add_command(
