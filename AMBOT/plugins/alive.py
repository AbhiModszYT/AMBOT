...
from AMBOT.DB.gvar_sql import gvarstat, addgvar
from AMBOT.plugins import *

# -------------------------------------------------------------------------------

ALIVE_TEMP = """
<b><i>⚡ 𝗣𝗺 𝗣𝗿𝗼𝘁𝗲𝗰𝘁𝗲𝗿 𝗔𝗠𝗕𝗢𝗧 ɨs օռʟɨռɛ ⚡</i></b>
<b><i>↼ Øwñêr ⇀</i></b> : 『 {hell_mention} 』
╭──────────────
┣─ <b>» Telethon:</b> <i>{telethon_version}</i>
┣─ <b>» 𝘈𝘔𝘉𝘖𝘛:</b> <i>{hellbot_version}</i>
┣─ <b>» Sudo:</b> <i>{is_sudo}</i>
┣─ <b>» Uptime:</b> <i>{uptime}</i>
┣─ <b>» Ping:</b> <i>{ping}</i>
╰──────────────
<b><i>»»» <a href='https://t.me/AbhiModszYT_Return'>[AbhiModszYT]</a> «««</i></b>
"""
...
async def hell_a(event):
    ...
    uptime = await get_time((time.time() - StartTime))
    am = gvarstat("ALIVE_MSG") or "<b>»» ⚡ 𝗣𝗺 𝗣𝗿𝗼𝘁𝗲𝗰𝘁𝗲𝗿 𝗔𝗠𝗕𝗢𝗧 ιѕ σиℓιиє ⚡ ««</b>"
    try:
