...
async def kk(event):
    ...
    o = "\n".join(_o)
    OUTPUT = f"""
<h1>List of Plugins in AMBOT:</h1>

<code>{o}</code>

<b><i>HELP:</b></i> <i>If you want to know the commands for a plugin, do “ .plinfo <plugin name> ”

<b><a href='https://t.me/AbhiModszYT_Return/115'>@AM_YTBOTT</a></b>
"""
    hell = await telegraph_paste("All available plugins in AMBOT", OUTPUT)
    await eor(event, f"[All available plugins in AMBOT]({hell})", link_preview=False)
...
async def send(event):
    ...
    omk = f"**• Plugin name ≈** `{input_str}`\n**• Uploaded by ≈** {hell_mention}\n\n⚡ **[AbhiModszYT]({chnl_link})** ⚡"
    the_plugin_file = "./AMBOT/plugins/{}.py".format(input_str.lower())
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "./AMBOT/plugins/",  # pylint:disable=E0602
                )
            )
            if owo != "-f":
...
async def uninstall(event):
    ...
    dir_path = f"./AMBOT/plugins/{shortname}.py"
    try:
...
CmdHelp("core").add_command(
    "install", "<reply to a .py file>", "Installs the replied python file if suitable to AMBOT's codes.`\n**🚩 Flags :** `-f"
).add_command(
    "uninstall", "<plugin name>", "Uninstalls the given plugin from AMBOT. To get that again do .restart", "uninstall alive"
).add_command(
    ...
    "cmds", None, "Gives out the list of modules in AMBOT."
).add_command(
    "repo", None, "Gives AMBOT's Github repo link."
).add_command(
