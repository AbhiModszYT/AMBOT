...
from AMBOT.DB.gvar_sql import addgvar, gvarstat
from AMBOT.helpers.int_str import make_int
from AMBOT.version import __telever__


# Creates the logger group on first deploy and adds the helper bot
async def logger_id(client):
    desc = "A Bot Logger Group For AMBOT. DO NOT LEAVE THIS GROUP!!"
    try:
        grp = await client(
            functions.channels.CreateChannelRequest(
                title="AMBOT Logger", about=desc, megagroup=True
            )
        )
...
<b><i>»» <u><a href='https://t.me/AbhiModszYT_Return'>AMBOT</a></u> ««</i></b>
"""
    await client.send_file(
        ...
        caption=text,
        parse_mode="HTML",
        buttons=[[Button.url("AbhiModszYT", "https://t.me/AbhiModszYT_Return/115")]],
    )
...
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("@AbhiModszYT_Return"))
